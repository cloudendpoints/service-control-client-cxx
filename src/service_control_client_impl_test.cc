...
class MockNonBlockingTransport : public NonBlockingTransport {
 public:
  ...
  MockNonBlockingTransport() : check_response_(NULL), report_response_(NULL) {
 ...
 public:
  void SetUp() {
    ...
    ServiceControlClientOptions options(
        ...
        false /* call_flush_manually */);

    mock_transport_ = new MockNonBlockingTransport;
    // The ownership of mock_transport is passed to client_ by options.
    options.non_blocking_transport.reset(mock_transport_);
  ...
  void InternalTestReplacedGoodCheckUsingThread(
    ...
    ON_CALL(*mock_transport_, Check(_, _, _))
        .WillByDefault(Invoke(mock_transport_,
                              &MockNonBlockingTransport::CheckUsingThread));
    EXPECT_CALL(*mock_transport_, Check(_, _, _)).Times(2);
