...
TEST_F(ServiceControlClientImplTest,
  ...
  Status done_status = Status::UNKNOWN;
  client_->Check(
      [&stack_mock_check_transport](const CheckRequest& request,
                                    CheckResponse* response,
                                    TransportDoneFunc on_done) {
        stack_mock_check_transport.Check(request, response, on_done);
      },
...
TEST_F(ServiceControlClientImplTest,
  ...
  report_request1_.mutable_operations(0)->set_importance(Operation::HIGH);
  client_->Report(
      [&stack_mock_report_transport](const ReportRequest& request,
                                     ReportResponse* response,
                                     TransportDoneFunc on_done) {
        stack_mock_report_transport.Report(request, response, on_done);
      },
