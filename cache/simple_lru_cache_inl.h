...
const int64_t kSecToMsec = 1000000;
...
class SimpleCycleTimer {
 public:
  ...
  static int64_t Now() {
    ...
    return static_cast<int64_t>(tv.tv_sec * kSecToMsec + tv.tv_usec);
  }
  // Return number of cycles in a second.
  static int64_t Frequency() { return kSecToMsec; }
...
int64_t SimpleLRUCacheBase<Key, Value, MapType,
  ...
  return kSecToMsec * (SimpleCycleTimer::Now() - head_.prev->last_use_) /
