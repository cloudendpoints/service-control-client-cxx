...
CheckAggregatorImpl::~CheckAggregatorImpl() { FlushAll(); }
...
int CheckAggregatorImpl::NextFlushInterval() { return options_.expiration_ms; }
...
void CheckAggregatorImpl::OnCacheEntryDelete(CacheElem* elem) {
  GOOGLE_LOG(INFO) << "OnCacheEntryDelete begin";
  if (!elem->HasPendingCheckRequest()) {
    delete elem;
    return;
  }

  GOOGLE_LOG(INFO) << "OnCacheEntryDelete 2";
  if (flush_callback_) {
    GOOGLE_LOG(INFO) << "OnCacheEntryDelete 3";
    CheckRequest request;
